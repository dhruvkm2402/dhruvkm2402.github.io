...
plugins. **`assets/js/site.js` is deliberately outside that bundle** — it's loaded directly from
...
possible for the custom layer plus the explicit overrides in `_dark.scss` — don't expect a token
